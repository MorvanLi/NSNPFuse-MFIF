...
class LpLssimLoss(nn.Module):
    ...
    # def image_gradient(self, input, gray=False):
...
def RMI(x, y, args):
    RMI = RMILoss().to(args.device)
    RMI = RMI(x, y)

    return RMI


def RMI_vi(input_vi, fused_result, args):
    RMI_vi = RMI(input_vi, fused_result, args)

    return RMI_vi


def RMI_ir(input_ir, fused_result, args):
    RMI_ir = RMI(input_ir, fused_result, args)
